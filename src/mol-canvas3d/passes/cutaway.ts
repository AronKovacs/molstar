...
import cutaway_jfa_step_frag from '../../mol-gl/shader/cutaway-jfa-step.frag';
import cutaway_init_frag from '../../mol-gl/shader/cutaway-init.frag';
import cutaway_copy_to_target_frag from '../../mol-gl/shader/cutaway-copy-to-target.frag';
...
function getCutawayInitRenderable(ctx: WebGLContext, depthTexture: Texture): CutawayInitRenderable {
    ...
    const shaderCode = ShaderCode('cutaway-init', quad_vert, cutaway_init_frag);
...
function getCutawayJfaRenderable(ctx: WebGLContext, cutawayTexture: Texture): CutawayJfaRenderable {
    ...
    const shaderCode = ShaderCode('cutaway-jfa', quad_vert, cutaway_jfa_step_frag);
    const renderItem = createComputeRenderItem(ctx, 'triangles', shaderCode, schema, values);
...
function getCutawayCopyToTargetRenderable(ctx: WebGLContext, depthTexture: Texture): CutawayCopyToTargetRenderable {
    ...
    const shaderCode = ShaderCode('cutaway-copy-to-target', quad_vert, cutaway_copy_to_target_frag);
    const renderItem = createComputeRenderItem(ctx, 'triangles', shaderCode, schema, values);
...
export class CutawayPass {
    ...
    public get target() {
        return this.supported ? this._target.texture : null;
    }

    constructor(private webgl: WebGLContext, width: number, height: number) {
        this._supported = CutawayPass.isSupported(webgl);
    ...
    private renderDepth(renderer: Renderer, camera: ICamera, scene: Scene) {
        this._target.bind();
